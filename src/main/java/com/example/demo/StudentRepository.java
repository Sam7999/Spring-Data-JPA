...
@Repository
@Transactional (readOnly = true ) // this is when we are not modifying the data only reading
public interface StudentRepository extends JpaRepository <Student, Long> {
    ...
    //BY DEFAULT  ALL QUERRY METHODS ARE TRANSACTIONAL WHEN WORKING WITH SPRING DATA JPA

    @Transactional  // we add this so we can execute the next query without an error // we are overriding the read only transaction
    // WE use it when we are modifying data
